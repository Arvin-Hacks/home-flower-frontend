...
const ProductDetails = () => {
    ...
    const { getCartData: cart, loading: cartLoading } = useAppSelector(store => store.cartReducer)
                ...
                <div className="flex flex-col lg:flex-row bg-white rounded-xl shadow-lg overflow-hidden max-w-4xl">
                    ...
                    <div className="flex flex-col p-6 space-y-4 lg:w-1/2">
                        ...
                        {cart?.data?.[0]?.items?.some((item: any) => item?.product?._id === product?.data?._id) ?
                            ...
                            :
                            <Button className="self-start mt-4 bg-red-500 text-white" onClick={() => handleAddtoCart(product?.data?._id)}>
                                Add to Cart
                            </Button>
                        }
                    </div>
